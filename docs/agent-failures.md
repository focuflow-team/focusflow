...
| F-2026-04-15-B | `tick` 내 `completed` 플래그가 항상 false — `onSessionComplete` 미호출 | `setState(updater)` 안에서 `completed = true`를 설정한 뒤 `setState()` 직후에 확인. React 18 자동 배칭으로 updater가 비동기 실행되므로 플래그가 반환 시점에 아직 false. | `stateRef`로 현재 state를 미러링하고 완료 조건을 `setState` 호출 전 판단. setState updater를 순수하게 유지하고 side effect(콜백 호출, RAF 제어)는 updater 밖에서 처리할 것. | 타이머/React |
